...
/*选中*/
        $("."+can.zi).each(function() {
            ...
            if (url.split(pddq).length > 1) {
                $(this).addClass('on');
                $(can.nuv).find(can.qingchu).before("<div rel=" + $(this).attr('rel') + " name=" + $(this).attr('name') + " href='javascript:;'>" + $(this).siblings('span').text() + $(this).text() + "</div> ")
            } else {
                $(this).removeClass('on')
            }
        })
...
/*点击清除*/
	$(can.nuv).find('div').click(function() {
    	var url = window.location.href;
        var pddq = $(this).attr('name') + "=" + $(this).attr('rel');
        if (url.split('&').length < 2) {
             url = url.replace('?' + pddq, '')
