...
public class PagingCreditCard {
	//分页查询sql语句select * from talblename limit startRows,rows
		...
		private QueryCreditCard queryCreditCard;//条件
		...
		public Integer getRows() {
			...
			/*if((rows+getStartRows())>getRowsCount()) {
				rows=getRowsCount()-getStartRows();
			}else {
				return rows;
			}*/
			return rows;
		...
		public QueryCreditCard getQueryCreditCard() {
			return queryCreditCard;
		}
		public void setQueryCreditCard(QueryCreditCard queryCreditCard) {
			this.queryCreditCard = queryCreditCard;
		}
